...
class Command(BaseCommand):
    ...
    def create_sample_basketball_data(self):
        ...
        BasketballStats.objects.filter(player_name__contains='Demo').delete()
        ...
        for i in range(20):  # Create 20 sample players
            player_name = f"{random.choice(first_names)} {random.choice(last_names)} (Demo)"
    ...
    def create_sample_football_data(self):
        ...
        # Clear existing sample data to avoid duplicates
        FootballStats.objects.filter(player_name__contains='Demo').delete()
